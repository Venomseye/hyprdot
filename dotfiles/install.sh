...
# Files that need a one-time, install-time substitution of the literal
# string __HOME__ for the real $HOME path. These are NOT matugen output
# (they're static config I wrote once, so they go through the normal
# backup-and-sync path above like anything else) - they just can't be
# shipped with the real path baked in, since I don't know your username at
# package-build time, and qt5ct's color_scheme_path setting does not expand
# $HOME or ~ on its own (confirmed against a real, working qt5ct.conf
# example - it needs a literal, fully-resolved absolute path).
HOME_SUBSTITUTION_FILES=(
    "qt5ct/qt5ct.conf"
    "qt6ct/qt6ct.conf"
)

needs_home_substitution() {
    local rel="$1"
    local f
    for f in "${HOME_SUBSTITUTION_FILES[@]}"; do
        [ "$rel" = "$f" ] && return 0
    done
    return 1
}
...
while IFS= read -r -d '' file; do
    ...
    # For files needing __HOME__ substitution, compare and copy against a
    # substituted TEMP copy, not the raw source (which still literally says
    # "__HOME__"). Otherwise these would look "different" from the already-
    # correctly-installed target on every single re-run - since the raw
    # source never matches what actually got installed - triggering a new
    # pointless backup every time even though nothing real had changed.
    effective_source="$file"
    if needs_home_substitution "$rel"; then
        effective_source="$(mktemp)"
        sed "s|__HOME__|$HOME|g" "$file" > "$effective_source"
    fi

    if [ -e "$target" ]; then
        if cmp -s "$effective_source" "$target"; then
            say "unchanged  $rel"
            [ "$effective_source" != "$file" ] && rm -f "$effective_source"
            continue
        fi
    ...
    run cp "$effective_source" "$target"
    [ "$effective_source" != "$file" ] && rm -f "$effective_source"
