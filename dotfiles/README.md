...
---
## Round 18: the four things you picked - desktop pieces, Qt theming, btop, wallpaper picker + OSD

### Missing desktop pieces

- **Polkit agent**: `hyprpolkitagent` added to autostart - Hyprland's own
  first-party agent (hyprwm org, official Arch `extra`), rather than a
  GNOME/KDE one that pulls in unrelated dependencies. Without any agent
  running, apps needing a GUI privilege prompt (GParted, some installers)
  were failing silently or hanging.
- **System tray**: added to waybar. `blueman-applet` has been in autostart
  since the very first version of this config with nowhere for its icon to
  render - this is that place.
- **Idle inhibitor**: a manual "don't lock/sleep right now" toggle in the
  bar, for video calls or anything that doesn't send its own inhibit signal.
  Icons verified against Font Awesome (`fa-eye`/`fa-eye-slash`, U+F06E/F070).
  Styled with a real CSS class (`#idle_inhibitor.activated { color: @urgent;
  }`) rather than a hardcoded colour - confirmed via real GTK cascade
  resolution that it correctly tracks the live variable, same rigor as the
  mic-colour fix in Round 15.

### Qt/Dolphin theming

Added `[templates.qt5ct]`/`[templates.qt6ct]` plus `qt5ct.conf`/`qt6ct.conf`
using Qt's own built-in **Fusion** style - deliberately not Breeze/Kvantum/
Darkly, since Fusion ships with Qt itself and needs no extra AUR package,
which meaningfully shrinks the dependency footprint I was originally
hesitant about back in Round 15.

Worth being direct about: the 21-colour Qt palette mapping (in
`qt-colors.template`) is a **reasoned judgement call, not a byte-verified
standard** the way the GTK template is. qt5ct's colour-scheme file format is
documented (an ordered list of 21 roles), but there's no single canonical
Material-role-to-Qt-role mapping to copy from the way `InioX/matugen-themes`
provides for GTK. If some specific Dolphin element looks off, that mapping
is the first thing to adjust.

One real technical snag, worth knowing about: `color_scheme_path` needs a
**literal, fully-resolved absolute path** - confirmed against a real,
working qt5ct.conf example, since qt5ct does not expand `$HOME` or `~` for
this setting. Since I don't know your username at package-build time, the
shipped files use a placeholder (`__HOME__`) that `install.sh` now
substitutes with your real home directory at install time - verified this
substitution is idempotent too (won't falsely mark the file "changed" on
every re-run just because the raw source still says `__HOME__`).

### More apps themed (btop)

Added a `btop` template. Honesty note: I verified the overall
`theme[key]="value"` syntax structure, not every individual key name against
btop's own source - it's a well-established, widely-forked format, but if
one specific graph or panel doesn't pick up a colour, that's the first place
to check.

Enabling it needed care: shipping a full `btop.conf` risked exactly the kind
of clobbering Round 17 just fixed - if you already have btop configured with
your own proc-sorting/update-interval preferences, syncing a whole file
would have overwritten them. Instead, the `btop` template's `post_hook` is a
surgical one-liner that touches only the single `color_theme` line -
replacing it if present, appending it if the file exists without one,
creating the file if it doesn't exist at all. Tested all three cases
end-to-end, including a config with pre-existing unrelated settings, and
confirmed only that one line ever changes.

### Wallpaper picker + volume/brightness OSD

**OSD**: `swayosd-server` added to autostart; the volume/brightness/mic-mute
hardware keys and waybar's own click-to-mute actions now go through
`swayosd-client` (confirmed exact command syntax against SwayOSD's own
README) instead of calling `wpctl`/`brightnessctl` directly, so you get an
actual on-screen popup instead of only seeing the change in a slider you
might not be looking at.

**Wallpaper picker**: `SUPER+SHIFT+W` then `p` opens a rofi menu with real
image thumbnails (confirmed via rofi's own `rofi-thumbnails(5)` man page -
the `thumbnail://` URI prefix) to jump straight to a specific wallpaper.

Important limitation I want to be upfront about: **wpaperd/wpaperctl has no
released, stable command to jump to one specific wallpaper by path.** The
only such feature I found (`wpaperctl set <monitor> <path>`) is an open,
unmerged pull request, not something to depend on. What wpaperd does
confirm supporting is hot-reloading its own config file for every setting.
So the picker works by briefly rewriting wpaperd's `path` setting to the one
chosen file, waiting for wpaperd to notice and switch (which fires the
normal matugen hook, same as any other wallpaper change), then immediately
reverting `path` back to the wallpaper folder so `l`/`h` cycling keeps
working afterward. It's a workaround for a missing feature, not a first-class
API - tested the underlying sed operations directly (including a filename
with spaces in it) and confirmed the folder path and every other wpaperd
setting survive untouched, but if a chosen wallpaper doesn't seem to switch,
try increasing `WAIT_SECONDS` in `wallpaper-picker.sh` first. One harmless
side effect: after first use, wpaperd's `path` setting changes from
`~/Pictures/Wallpapers` (tilde) to the equivalent fully-expanded absolute
path - functionally identical, just cosmetically different if you inspect
the file.

### Also updated

`install.sh --with-deps` now includes `hyprpolkitagent`, `swayosd`, `qt5ct`,
`qt6ct`, `btop`, and `htop` (the latter two were already bound to click
actions in waybar but were never actually in the dependency list).
...
rofi-wayland kitty mako playerctl jq inotify-tools grim slurp satty
wl-clipboard cliphist brightnessctl wireplumber adw-gtk-theme
hyprpolkitagent swayosd qt5ct qt6ct btop htop`, plus `zscroll` for the
scrolling title (the script degrades to unscrolled text without it) and the
`JetBrainsMono Nerd Font` / `Symbols Nerd Font` families. Run
`./install.sh --with-deps` to install everything above in one go (Arch-only;
see Round 16 for exactly what that does and doesn't cover).
...
override. Qt/KDE apps (Dolphin and similar) now have their own theming path
via `qt5ct`/`qt6ct` (Round 18) - set `QT_QPA_PLATFORMTHEME=qt5ct` (already
in `hyprland.lua`) and the generated colour scheme takes over from there.
