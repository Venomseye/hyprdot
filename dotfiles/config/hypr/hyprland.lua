...
hl.on("hyprland.start", function()
	...
	-- [NEW] Polkit authentication agent. Without one running, any app that
	-- needs a GUI privilege-elevation prompt (GParted, some installers,
	-- certain mount operations) either fails silently or hangs waiting for a
	-- prompt that never appears - there was nothing here to show it before.
	-- hyprpolkitagent is Hyprland's own first-party agent (hyprwm org on
	-- GitHub, official Arch `extra` package), so it's the natural fit here
	-- rather than a GNOME/KDE-specific one that pulls in unrelated
	-- dependencies. It has sane defaults and needs no config file.
	hl.exec_cmd("hyprpolkitagent")

	-- [NEW] SwayOSD's background server. Must be running before anything
	-- calls `swayosd-client` (see the volume/brightness/mic-mute binds
	-- further down) - the client just sends a request to this server, which
	-- is what actually draws the on-screen popup.
...
hl.define_submap("", function()
	...
	-- [NEW] Browse and pick a specific wallpaper with visual thumbnails,
	-- instead of blindly cycling with l/h. See wallpaper-picker.sh for how
	-- this actually sets a chosen file (wpaperd/wpaperctl has no stable
	-- "jump to this exact wallpaper" command as of this writing).
	hl.bind("p", hl.dsp.exec_cmd("~/.config/hypr/wallpaper-picker.sh"))
...
--
-- [NEW] Routed through swayosd-client rather than calling wpctl/brightnessctl
-- directly, so pressing these actually shows a visual popup - previously the
-- only feedback was whatever you could see in waybar's volume slider, which
-- isn't very visible at a glance. swayosd-server must be running (added to
-- the autostart block above) for the client to have anything to talk to.
-- Command syntax confirmed against swayosd's own README (ErikReider/SwayOSD).
hl.bind(
